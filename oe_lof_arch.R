...
allLoFs<-read.csv("outputs/arch_allLoFs.csv")
hetLoFs<-read.csv("outputs/arch_hetLoFs.csv")
homLoFs<-read.csv("outputs/arch_homLoFs.csv")
transitionLoFs<-read.csv("outputs/arch_transitionLoFs.csv")
transversionLoFs<-read.csv("outputs/arch_transversionLoFs.csv")

#number line plot (hom, het) 
pdf("figs/oe_arch_homhet.pdf")
xmin = min(homLoFs$oe_lof,hetLoFs$oe_lof)
xmax = max(homLoFs$oe_lof,hetLoFs$oe_lof)
plot(0:1, axes=FALSE,type="n",xlab="oe_lof",ylab="", main="Archaic oe_lof Visualization", xlim=c(xmin,xmax))
axis(1,pos=0) 
...
ymax = max(hetd$y,homd$y,alld$y)
plot(hetd, col="gold", main="Archaic Density Plot", xlim=c(xmin,xmax), ylim=c(0,ymax))
...
plot(transitiond, col="darkorchid", main="Archaic Density Plot", xlim=c(xmin,xmax), ylim=c(0,ymax))
